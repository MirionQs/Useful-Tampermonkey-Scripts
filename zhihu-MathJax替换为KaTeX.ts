...
(function () {
	...
	if (url.startsWith('https://zhuanlan.zhihu.com/')) { // 文章
		renderByKatex(document.getElementsByClassName('Post-RichTextContainer')[0] as HTMLElement)
	}
	...
	function renderByKatex(element: HTMLElement) {
		Array.prototype.forEach.call(element.getElementsByClassName('ztext-math'), (el: HTMLElement) => {
			normalize(el)
	...
	function normalize(element: HTMLElement) {
		element.parentElement!.childNodes.forEach(el => { // 清除空文本节点
			el.nodeType === Node.TEXT_NODE && (el as Text).data.trim() === '' && el.remove()
		})
	...
	function isDisplayFormula(element: HTMLElement) {
		...
		if ([element.previousSibling, element.nextSibling].every(el =>
			el === null || el.nodeType === Node.ELEMENT_NODE && ((el as Element).tagName === 'BR' || (el as Element).className === 'ztext-math')
		)) { // 两侧为空
			return true
		}
		return false
